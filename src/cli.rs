...
/// How the grackle process should run.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum RunMode {
    /// Daemon mode (default): listen on the Unix-socket IPC for grackctl.
    #[default]
    Daemon,
...
pub struct RunOptions {
    ...
    /// `~/.config/grackle/config.toml` is used when present.
...
/// Default path for the config file (`~/.config/grackle/config.toml`).
#[must_use]
